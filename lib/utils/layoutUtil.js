...
// helpers /////
export function coordinatesToPosition(row, col, verticalOffset = 0) {
  return {
    width: DEFAULT_CELL_WIDTH,
    height: DEFAULT_CELL_HEIGHT,
    x: col * DEFAULT_CELL_WIDTH,
    y: row * DEFAULT_CELL_HEIGHT + verticalOffset
  };
}

export function getBounds(element, row, col, attachedTo, verticalOffset = 0) {
  ...
  if (!attachedTo) {
    return {
      ...
      y: row * DEFAULT_CELL_HEIGHT + (DEFAULT_CELL_HEIGHT - height) / 2 + verticalOffset
    };
  }

  const hostBounds = getBounds(attachedTo, row, col, null, verticalOffset);
