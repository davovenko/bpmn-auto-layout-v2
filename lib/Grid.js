export class Grid {
  constructor() {
    this.grid = [];
    this.horizontalLayout = true; // Всегда используем горизонтальную компоновку
  }
  ...
  addAfter(element, newElement) {
    ...
    // Добавляем элемент в правую позицию (горизонтальная компоновка)
  ...
  addBelow(element, newElement) {
    ...
    // Если под текущим элементом уже есть другой элемент,
    // создаем новую строку
    if (this.grid[row + 1][col]) {
      this.grid.splice(row + 1, 0, []);
    }
    
    // Добавляем элемент снизу, но в том же столбце
